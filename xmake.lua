...
rule("c.def_gen")
    ...
    on_buildcmd_file(function (target, batchcmds, sourcefile, opt)
        ...
        -- cauchy product
        if prod_defs and #prod_defs > 0 then
            for _, lst in ipairs(prod_defs) do
                local cache_defs = {}
                for _, item in ipairs(lst) do
                    if item == "none" then
                        table.join2(cache_defs, defs_result)
                    else
                        for _, res in ipairs(defs_result) do
                            table.insert(cache_defs, table.join(res, {item}))
                        end
                    end
                end
                defs_result = cache_defs
            end
        end
        ...
        for _, res in ipairs(defs_result) do
            ...
            if #res > 0 then
                batchcmds:show_progress(opt.progress, "${color.build.object}compiling.%s %s with defines %s", get_config("mode"), sourcefile, table.concat(res, " "))
